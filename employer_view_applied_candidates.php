  ...
  $jobs=mysqli_query($con,$sql) or die('Error in the sql query1');
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST')
	{
		...
		if(isset($_POST['select']))
		{
			...
			$sql="update applied_candidates set flag=1 where acid='".$_POST['select']."'";
          ...
          <?php
            ...
            if(isset($_POST["jobid"]))
            {
              ...
              $jobid=$_POST['jobid'];
							...
							$sql="select * from applied_candidates where job_id=$jobid AND flag=0  order by score DESC";
										...
										else
										{
											while($row = mysqli_fetch_array($applied_candidates))
											{
												...
												echo '<tr>';
													...
													echo '<td>' . $row2[2] . '</td>';
													echo '<td>' . $row[3] . '</td>';
													echo '<td> <button type="submit" class="primary" name="view" value="' . $row[1] . '">View</button>  <button type="submit" class="primary" name="select" value="' . $row[0] . '">Select for Interview</button><input type="hidden" name="acidtext" value="' . $row[0] .'"/></td>';
												echo '</tr>';
											}
										}
									}
